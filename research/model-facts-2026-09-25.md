...
## Second pass (2026-09-25, later — Claude Code harness docs + missed unsourced claims)

- **New source, A-SA:** Claude Code / Agent SDK docs, [Subagents in the SDK](https://code.claude.com/docs/en/agent-sdk/subagents) and [Claude Code subagents](https://code.claude.com/docs/en/sub-agents).
  - Subagent model resolution order: per-invocation `model` param → the agent definition's `model` frontmatter (`inherit` = main model) → `CLAUDE_CODE_SUBAGENT_MODEL` env var → **the main conversation's model**. There is no automatic cheaper-tier default — an unspecified subagent runs on the same model as the controller unless something explicitly overrides it.
  - The `claude_code` system prompt preset adds an anti-over-delegation line to the system prompt only "when the model is Opus 5" — this is stated for Opus 5 specifically, not Opus 5.5. Added to `delegate-to-subagents/opus-5-5.md`: the harness default that dampens delegation for Opus 5 does not fire for 5.5, so the skill's own check is the only thing doing that work unless the Opus 5 prompting guide's instruction is added explicitly.
  - No GPT-6 Sol-specific prompting guide exists; confirmed 404 on the pattern OpenAI uses for other per-model pages. `developers.openai.com/api/docs/guides/latest-model` explicitly frames its prompting guidance as "address[ing] behavior observed with GPT-6 Astra."
- **Fixed on this pass:** a wave of unsourced "reasoning effort peaks below the top setting" / "more effort doesn't reliably improve X past a certain point" claims survived the first audit's regex sweep in `gpt-6-sol.md` (adversarial-review, bite-sized-plan, chestertons-fence, clarify-the-real-problem, interface-contracts, measure-before-optimizing, observability-and-instrumentation, premortem, senior-engineer-mindset, simplicity-budget, verifiability-first), `gpt-6-luna.md` (premortem), and `gpt-6-astra.md` (simplicity-budget) — none of these appear in any OpenAI source checked. Replaced with the defensible, source-free claim that a higher setting costs more without a *guaranteed* matching gain, so it's worth measuring rather than defaulting to max — true by construction, not a specific unverifiable benchmark result.

## Third pass (2026-09-25, later — Anthropic's effort doc)

- **New source, A-EFF:** Anthropic, [Effort](https://platform.claude.com/docs/en/build-with-claude/effort).
  - The model-agnostic effort-levels table lists `low`'s typical use case as "Simpler tasks that need the best speed and lowest costs, **such as subagents**." Added to `delegate-to-subagents/opus-5-5.md`, `fable-5-1.md`, and `sonnet-5.md`: model tier and effort are two separate levers for a dispatched worker, and Anthropic's own guidance defaults a worker's effort to `low` regardless of tier, raised only when a leg needs more depth. Not added to the GPT-6 files — OpenAI's `reasoning.effort` scale has no equivalent documented recommendation tying it to subagent/worker roles.
  - Per-model recommended-effort sections confirm defaults already in the specs table (Fable 5.1 `high`, Opus 5.5 `medium`, Sonnet 5 `high`) and add nothing that changes an existing claim.
- Checked `developers.openai.com/cookbook/examples/gpt-5/codex_prompting_guide`: it's the gpt-5.3-codex-era guide, not GPT-6 Sol/Luna — confirms again that no Sol/Luna-specific prompting guide exists, but nothing in it is safe to attribute to the current models.
