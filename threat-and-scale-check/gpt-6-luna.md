...
You're built for narrow, high-volume, well-specified work, not for the judgment calls this skill sometimes needs — whether a trust-boundary gap is worth escalating, whether a layered-defense tradeoff is acceptable, whether a scale concern is real or premature. Where the check is mechanical and pattern-based — does this string flow unescaped into a SQL query, is there a network call inside a loop, is there exactly one validation point or two — run it directly and report a concrete yes/no with the specific line. Where the answer requires weighing risk (is this trust boundary crossing a team line, does a breach here carry regulatory exposure, is one layer of defense enough), flag the question for a more capable tier instead of resolving it yourself.
...
- Is there a DB query or network call inside a loop? (N+1) — this is a pattern you can spot directly: a query or request literally inside a loop body
- Is everything loaded into memory at once? Can it be streamed or batched?
- But **don't optimize without measuring first** — confirm the bottleneck is actually here before touching it. Whether a fix is actually warranted at the current scale is a judgment call worth surfacing rather than deciding alone

## Layered defense

A structure where the next layer catches a failure is more realistic than betting everything on one perfect defense.

- If this safeguard fails, is it an immediate incident, or is there another layer behind it? Report what you find (is there a second layer, yes or no) and let a more capable tier weigh whether that's sufficient
...
Treat "does this action need a permission check" and "is validation enforced at only one point" as literal checks to run against the diff, not rhetorical prompts — trace the actual input path and report exactly what you found, rather than assuming it's fine because nothing obviously looks wrong.
