...
- **Set effort explicitly when a handoff decision is non-trivial.** Effort — not a thinking toggle — is the depth control here, and it defaults to `medium` (Opus 5 defaulted to `high`; Anthropic's testing found 5.5 at `medium` matches or beats Opus 5 at `high`). Deciding what's safe to drop versus what must survive to a file is a judgment call worth doing deliberately; if a large handoff has shown better results at a higher level, the caller sets that for the request — it can't be raised mid-turn.
- **Brevity in a handoff note doesn't come for free.** Response length doesn't reliably shrink just from lowering effort (carried over from Opus 5 guidance) — if the handoff needs to be terse (a one-line pointer, not a paragraph), say so explicitly rather than assuming a short deliverable follows naturally.
- **Trust your own self-check on "did I actually drop what I said I'd drop."** Opus 5 guidance, which Anthropic says still applies to 5.5, says the model verifies its own work unprompted and that added verification instructions cause over-verification; there's no need to add a second explicit "double-check you didn't leave the whole log pasted in" pass on top of the rules above — that's wasted tokens, not more reliability.
