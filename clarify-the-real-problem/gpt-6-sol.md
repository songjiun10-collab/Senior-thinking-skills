...
- Is a one-line request actually hours of work? **Reading it narrow reads as ignoring it** — scope it broad, then confirm, rather than interpreting it small. **This matters on this model**: it tends to follow instructions fairly literally, so an underspecified one-liner is more likely to get executed exactly as typed — narrowly — instead of generously expanded to what was probably meant. "Scope it broad, then confirm" has to be a deliberate step here, not a hopeful default.
