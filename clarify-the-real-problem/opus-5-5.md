...
**Building the wrong thing, thoroughly, is the specific failure mode to watch here.** Opus 5 guidance (still applicable to 5.5, per Anthropic) documents expanding a task's scope — adding unrequested steps or applying its own judgment about what the task should be — and Opus 5.5 "tends to get to work quickly." Clarifying the real problem *before* building keeps that energy pointed at the right target. Do the clarifying pass first; check in only when different readings would lead to materially different work.

**Keep this pass itself proportionate.** Thinking is always on and effort defaults to medium — that's usually enough for a clarifying pass, since the skill's value is in the questions asked, not in exhaustive reasoning about every possible interpretation. Reserve raising effort for genuinely high-stakes ambiguity (the Distinguished/Fellow or Executive angles above), not for routine "what did they mean" judgment calls. And if the answer, once clarified, is short, say so briefly — response length doesn't reliably shrink on its own (carried over from Opus 5 guidance), so a one-line clarification followed by a one-line confirmation has to be written that way deliberately, not padded out by default.

**This clarifying step is usually worth doing directly rather than delegating.** It needs the same conversational context the rest of the session has (what was actually said, what's been discussed before); dispatching it to a subagent that doesn't share that context tends to lose the nuance the pass exists to catch.
