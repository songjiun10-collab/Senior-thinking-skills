...
**Building the wrong thing, thoroughly, is the specific failure mode to watch here.** Left unconstrained, this model tends toward extra scope — more files, more abstraction, more unrequested flexibility than the request called for. Clarifying the real problem *before* building is exactly what keeps that instinct pointed at the right target instead of over-engineering a solution to the wrong problem. Do the clarifying pass first; don't let eagerness to start building skip past it.

**Keep this pass itself proportionate.** Thinking is always on and effort defaults to medium — that's usually enough for a clarifying pass, since the skill's value is in the questions asked, not in exhaustive reasoning about every possible interpretation. Reserve raising effort for genuinely high-stakes ambiguity (the Distinguished/Fellow or Executive angles above), not for routine "what did they mean" judgment calls. And if the answer, once clarified, is short, say so briefly — response length doesn't shrink much on its own at this tier, so a one-line clarification followed by a one-line confirmation has to be written that way deliberately, not padded out by default.
