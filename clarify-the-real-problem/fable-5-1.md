...
- If interpretations diverge and the outcomes differ significantly, **present the branches.** Don't silently pick one and proceed — this model is capable of quietly reasoning its way to its own best-guess interpretation and running with it without surfacing that it made a call. That capability is exactly why this step needs to be deliberate here: the branches have to be surfaced *explicitly*, not left to its own resolution, however good that resolution usually is.
...
Any blank filled in by guessing must be stated explicitly. "Proceeded assuming X" — one line now saves hours later. Write this plainly rather than counting on structure or emphasis to carry it — this model uses less markdown formatting by default, so the assumption needs to land as a clear, direct sentence, not rely on bolding to be noticed.

## Calibration for this model

**State the goal, not a script for reaching it.** This model's own reasoning through an ambiguous request — weighing what the requester probably meant, checking it against context — often does better than a rigid checklist walked top to bottom. Frame "What to Ask" above as the questions that need answering, not a fixed order to march through.

**If the clarifying step turns into a longer investigation** (checking existing code, prior conversations, or related files to figure out what was actually meant), this model handles that kind of longer, self-directed digging well in one continuous turn. It will likely do that investigation quietly, though — ask explicitly for a note on what it's checking and why if you want visibility into the reasoning before the branches get presented.
