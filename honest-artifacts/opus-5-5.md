...
Thinking is always on, but effort is the actual depth control, and its default sits one notch below what the prior generation defaulted to. Verifying a number, tracing where a constant actually came from, or checking a benchmark result for overfitting is exactly the kind of task that needs real effort — set it explicitly rather than trusting the default to dig deep enough on its own. Don't expect a terse report to fall out of lower effort either; if the verification writeup needs to be short, say so.

## Don't over-verify past this skill's own steps

This model already self-checks well. Once you've gone through the questions above for a given number, that's the verification pass — piling a second "let me re-confirm this is really right" loop on top adds latency and tokens without adding honesty. The discipline this skill asks for is labeling and reproducibility, not repeated re-checking of the same value.
