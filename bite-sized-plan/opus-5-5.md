...
- **Watch scope expansion here specifically.** Opus 5 guidance, which Anthropic says still applies to Opus 5.5, documents a tendency to add steps that weren't requested — and "one responsibility per file" can read as license to add files and abstractions the task doesn't need. It's a floor for clarity, not a target to maximize. If the existing codebase uses fewer, larger files, match that; don't split further just because splitting is available.
...
| "This plan needs a config system / plugin layer / extra abstraction to be safe" | That's scope expansion talking, not the task. If nothing in the request asked for it, it doesn't belong in the plan |

## Calibration for this model

Thinking is always on and effort defaults to `medium`, which in Anthropic's testing matched or beat Opus 5 at `high` on coding work — usually enough for planning. If a large, hard-to-reverse plan has shown a real gain at a higher level, have the request that sketches the file layout (step 1) and sizes tasks (step 2) run at it; effort is set by the caller, not mid-turn. Lower effort doesn't reliably shorten visible output (carried over from Opus 5 guidance), so brevity in the final plan doc still has to be asked for directly if that's the goal.

Producing this plan is itself a good candidate for doing directly rather than delegating to a subagent — Opus 5 guidance (still applicable to 5.5, per Anthropic) documents delegating to subagents more readily than prior models, including for planning work that's faster done in the main thread since it needs the same context the rest of the session already has. Save delegation for executing the plan's independent tasks once they're written, not for writing the plan itself.

If a review or self-check pass over the finished plan feels tempting, know that Opus 5 guidance (still applicable to 5.5) says the model verifies its own work unprompted and that added verification instructions cause over-verification — an extra "double check this plan is complete" instruction on top of the Scope check and Warning signs above is likely to cost tokens without finding anything those two didn't already catch.
