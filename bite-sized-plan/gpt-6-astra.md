...
Write the trigger for each action explicitly — "run it" should say which command, "confirm it fails" should say what output confirms that. Underspecifying a step here isn't a shortcut, it's where the plan actually breaks.
...
- A completion condition — what state means this task is done, stated concretely enough to check without further judgment

State the completion condition for the plan as a whole up front, too, not just per task: what does "the plan is fully executed" mean — every task's tests green, a specific end-to-end check passing, something else? Left unstated, a plan executor tends to be more tentative than useful about declaring the whole plan finished, checking in partway rather than working the full list through; naming the finish line lets it run the plan to completion and then verify, rather than pausing task-by-task to ask if it should continue.
...
| "The step's intent is obvious, I don't need to spell out the exact command/trigger" | An implicit step is a gap, not a shortcut |
| "I should check with the user before starting task 2" | Only if task 2's completion condition or scope is genuinely undefined — otherwise infer intent from the plan and keep moving through it |

## Calibration notes for this model

- **State an explicit exploration scope for planning itself** — which parts of the codebase get read before the file layout is sketched, and where that reading stops. Left open-ended, this model can either under-explore (missing an existing pattern to follow) or over-ask about how much to look at; naming the scope up front avoids both.
- **Don't add "make sure you test each step" instructions** — the "run it, confirm it" steps already exist for a reason (they're the acceptance check a human or reviewer reads), but a separate reminder to "actually verify this works" is redundant: this model tests and verifies its own output without being told to.
- **Delegating individual tasks to subagents**: this model under-delegates by default. If the plan is meant to be split across multiple subagents or sessions, say so explicitly per task ("this task is meant to be handed to a subagent with no other context") — don't rely on the plan's shape alone to trigger delegation.
- **Writing style**: keep the plan and its task descriptions in clear, direct prose, using the numbered-step lists only where steps are genuinely sequential (as above) — not as a default format for descriptive text. Avoid slop phrasing ("delve into," "leverage," "it's worth noting," invented compound terms, "X, not Y" contrasts) in task descriptions; state the action plainly.
