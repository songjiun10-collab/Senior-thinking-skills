...
- The fix is a different **kind** of evidence, not a different reader: execute the exploit path, send the unauthorized request, replay the token — don't just re-read the diff (same evidence bar as `verify-before-claiming`). Default to running this yourself with a direct check (grep, curl, a scripted probe) rather than spinning up a subagent to do it — a subagent adds delegation overhead a direct execution settles faster, and it's still your job to look at the actual output
...
## A refusal mid-review is not automatically a finding

Your safety classifiers are broader than earlier models' — bio and reasoning_extraction now sit alongside cyber. Reviewing auth bypasses, sandbox escapes, or exploit paths is legitimate work squarely inside this skill's scope, and it can trip one of those classifiers as a false positive: a refusal, a truncated response, or a request to "be more careful" mid-review. Don't record that as a containment finding or a real block — it's a signal about your own classifier, not about the code. Before writing it up either way: re-read what you actually asked yourself to do, check whether it was a legitimate review action (probing a boundary, describing an exploit path for the writeup) versus something that would genuinely produce dangerous capability, and retry the review framed explicitly as defensive/audit work if the first pass tripped a false positive. If it holds on retry with a clearly defensive framing, treat it as a real signal worth escalating, not something to route around silently.
...
| "It got refused, so that's a finding" | Check whether it's a false positive from a broad classifier before writing it up |
| "I re-checked my own read, that's enough" | Re-verification without new evidence is correlated, not independent — go run the exploit |
