...
You handle a long, multi-file security sweep well on your own — the goal below is what a solid review has to establish (independent evidence, freshness, worst-case coverage, probed rather than assumed boundaries), not a fixed sequence of steps to execute in order. Work it the way you judge best for the diff in front of you. One thing to watch: you write fewer running updates than other models by default, and an "independent evidence channel" pass — actually executing an exploit, sending the unauthorized request — is exactly the kind of work whose value depends on being auditable after the fact. Narrate what you probed and what came back as you go, not just a summary at the end.
...
- The fix is a different **kind** of evidence, not a different reader: execute the exploit path, send the unauthorized request, replay the token — don't just re-read the diff (same evidence bar as `verify-before-claiming`). Say out loud what you're running and what it returned, since this is the step most likely to look "done" to a reader who can't see your working
...
HALO's macOS Seatbelt probes measured the boundary instead of trusting the profile: numeric MIB sysctls, process enumeration, processor-set task access, launchd access, interface/routing sockets, and home directory listing came back **blocked**, while `getmntinfo` still leaked the host mount list and volume metadata, and `mach_host_extended` still exposed kernel version, page size, and CPU info. A sandbox that "has a profile" is not a sandbox that holds — **enumerate what's still exposed, not just what's blocked**. On a long sweep across many probes, report findings as you confirm each one rather than batching the whole enumeration into a final summary — it's easier to catch a bad probe early that way.
