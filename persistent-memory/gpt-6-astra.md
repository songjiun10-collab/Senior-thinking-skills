...
- The trigger conditions above ("the same correction has come up more than once," "starting a task type you expect to repeat") are the actual criteria — apply them as written rather than waiting for an explicit "remember this for next time." An underspecified moment (a correction given once, without being told to persist it) still counts if it matches the pattern; act on it directly rather than asking whether to write it down.
- **State explicitly when a recurring task should start with `show` before proceeding** — left to infer it, checking memory before a repeat task is a step worth calling out as part of the task's stated completion criteria, not something to assume happens implicitly.
- A single append or a single review of one topic file is a direct, in-session read-and-edit — there's no benefit to delegating it to a subagent, and doing it directly is faster.
- Write each entry as one plain, direct sentence — the current state, not a contrastive "not X, but Y" framing of what changed. Keep the file itself free of headers, subsections, or a taxonomy of correction types unless the underlying task genuinely calls for that structure; a flat list of dated lines is the whole format.
