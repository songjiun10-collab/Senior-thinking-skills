...
- **This tier fits the mechanical side of this skill well: running `show`, `append`, or `list` exactly as instructed, and formatting a given correction into one dated line.** It's a good fit for a high-volume, narrow use of the CLI itself.
- The judgment calls this skill is really about — whether a correction will recur, whether it belongs in `CLAUDE.md` versus a memory file, whether two entries genuinely conflict or just overlap — are less this tier's strength. When those calls are ambiguous, prefer a stronger tier to make them, or make the placement explicit for this tier rather than leaving it to infer.
- Keep reviews and appends short and literal: write the one line given, don't expand it into extra context or structure. This tier's value here is doing the narrow, well-specified write quickly and cheaply, not deciding what belongs in memory in the first place.
