...
This checklist runs on its own as part of finishing the work — it doesn't need a separate "make sure you test this" instruction layered on top.
...
- **State completion criteria and scope explicitly before instrumenting a large surface.** For a service with many entry points, say up front what "done" means (the verification gate above satisfied for each new signal) and which entry points, queues, or external calls are in scope. Left unstated, the default here is to stay tentative about calling the pass finished rather than over-running it — a stated bar and boundary fixes both directions.
- Instrumentation work has clear implicit triggers — "add retries," "add a queue," "call this external service" — that should pull in this skill's rules even when the request doesn't say "observability" outright. Treat those as explicit triggers for the full rule set (correlation IDs, RED metrics, redaction).
- **Act on an implied instrumentation need rather than asking whether to add it.** Given a task that clearly needs new telemetry (a new retry path, a new external call), draft the on-call questions and the signals yourself and proceed, rather than checking in first about whether observability is wanted here.
- **State explicitly when checking several services or entry points for existing telemetry gaps should be split across subagents.** That kind of sweep is naturally parallel; left unstated, the default leans toward doing the whole sweep directly.
- **A refusal in this territory may be a false positive.** Instrumenting for security incidents, auth failures, or abuse detection is legitimate operational engineering. If a request in this space is declined, check whether it's actually defensive/operational work before treating it as blocked for cause — this tier's own judgment about what's safe to build is generally reliable here, and the old, more heavily prohibitive framing some guidance uses for this kind of work doesn't need to be taken as the final word.
- Write event names, questions, and rationale in direct prose. Keep the tables and enumerated rules above as-is — they're genuinely parallel items — but don't turn a single design rationale into a bulleted list where a sentence says it more clearly.
