...
- **Set effort explicitly for tracing a hard-to-explain incident.** Your default effort is `medium`; a genuinely tangled cross-service trace or an ambiguous incident needs more depth than that default gives it, and thinking stays on regardless — effort is the knob that actually changes here.
- **Run the verification gate once, thoroughly, and stop.** You already self-check well; re-confirming "yes the redaction really worked" three separate times past the checklist above is wasted latency, not extra safety.
- **Watch for over-engineering the instrumentation itself** — a generic telemetry abstraction layer, a configurable exporter framework, or schema flexibility nobody asked for. The rules above already specify what's needed; add only that.
- **A refusal on this skill's territory may be a false positive.** Instrumenting for security incidents, auth failures, or abuse detection is legitimate engineering work that can trip broader safety classifiers on this tier. If a request in this space gets refused, check whether it's actually a defensive/operational task before assuming it's blocked for cause.
