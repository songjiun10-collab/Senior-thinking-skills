...
- Instrumentation work has clear implicit triggers — "add retries," "add a queue," "call this external service" — treat those as reasons to apply the rules above even without the word "observability."
- **This tier fits one well-specified leg of an instrumentation task, not the whole design.** Use it to add a single structured log line matching a given schema, wire a single named metric with a given label set, or check one log stream for an unredacted field against an allowlist — narrow, checklist-shaped work. Deciding what the on-call questions should be, or which signal a genuinely ambiguous case needs, is judgment-heavy work this tier isn't the right fit for; hand that decision to a stronger tier and use this one to execute the resulting spec.
- When given a full verification-gate checklist to run, work through it item by item against the actual output rather than summarizing from expectation — it's well-suited to that kind of literal, bounded check.
- Keep reasoning effort low here; the value of this tier is speed and cost on narrow, well-specified telemetry tasks, and there's little judgment-quality upside from raising effort on a task that's already fully specified.
