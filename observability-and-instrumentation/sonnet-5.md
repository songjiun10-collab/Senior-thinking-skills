...
- Instrumentation work involves clear implicit triggers — "add retries," "add a queue," "call this external service" — that should pull in this skill's rules even when the request doesn't say "observability" outright. Treat those as explicit triggers for the full rule set above (correlation IDs, RED metrics, redaction) rather than waiting for the task to spell out "add telemetry too."
