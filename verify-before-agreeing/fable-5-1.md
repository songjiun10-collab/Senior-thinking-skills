...
Treat this as a goal to reason through, not a checklist to execute mechanically — working out whether a claim holds and how a suggestion interacts with everything else in the codebase is exactly the kind of open-ended reasoning this model does well without being walked through each step:
...
## Visibility during long review passes

Working through a long stack of review comments is exactly the kind of extended task this model handles well autonomously, but it tends to stay quiet while doing it. When a person is waiting on the outcome of the evaluation — not just the final "applied" or "pushed back" — say up front that progress updates are wanted per item (which claim is being checked, what was found) rather than a single verdict at the end.
