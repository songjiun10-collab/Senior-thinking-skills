...
This tier is the workhorse for grinding through a review queue — cheap enough to run as an unattended loop over many items, and it adopts the same clearer, lower-jargon collaboration style as the flagship. Two things to set deliberately: don't default reasoning effort to the top setting for routine items — effort past a moderate level doesn't reliably improve outcomes here, so tune it per workload rather than maxing it out by default. And because this tier trails the flagship on resisting pressure to route around a warning or guardrail, any review pass where this model is deciding on its own whether to apply a suggestion that touches a safety check, permission boundary, or explicit warning should get an independent spot-check before it's trusted — don't hand it the kind of judgment call reserved for the flagship tier.
