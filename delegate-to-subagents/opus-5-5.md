...
- **This is the check that matters most for you specifically.** Opus 5 is documented to delegate to subagents more readily than prior models, and Anthropic says Opus 5 guidance remains a reasonable starting point for Opus 5.5 — so treat the pull toward delegating tasks a direct grep or file read would settle faster as real, not a style choice, and this skill's whole "decide whether to delegate first" step exists to catch it before it fires. If the honest answer to "could I just grep/read this myself in under a minute" is yes, do that — don't dispatch a subagent for it because the instinct to delegate is already pointed that way.
- **In Claude Code specifically, the built-in damping only fires for Opus 5, not you.** Claude Code's `claude_code` system prompt preset adds a line telling the model not to call the Agent tool unless asked — documented as triggering "when the model is Opus 5." Running as Opus 5.5, that automatic line isn't added, so the check above is doing work no harness default is doing for you. If you want it enforced rather than just self-applied, add the [Opus 5 prompting guide's delegation instruction](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5#controlling-subagent-spawning) to the system prompt yourself, and set `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`/`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` (or the SDK's `max_budget_usd`) as a hard backstop — a prompt instruction only steers, it doesn't cap.
