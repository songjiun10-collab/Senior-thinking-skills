...
- **Pick the worker's model tier by subtask difficulty, not by what the controller happens to run on.** A narrow, mechanical leg (grep-and-report, one well-specified file edit) doesn't need the flagship tier; a genuinely ambiguous or hard-to-reverse leg does. See `senior-engineer-mindset`'s "Model-aware calibration" table for the tier breakdown this is based on.
- **Opus-class models delegate more eagerly than earlier ones, including for tasks a direct grep or file read would settle faster** — a documented tendency worth knowing about even when it's not your own instinct running the controller.
