...
- **Pick the worker's model tier by subtask difficulty, not by what the controller happens to run on.** A narrow, mechanical leg (grep-and-report, one well-specified file edit) doesn't need the flagship tier; a genuinely ambiguous or hard-to-reverse leg does. As a rule of thumb: cheapest/fastest tier for narrow, well-specified legs; the mid-tier as the default workhorse for most legs; reserve the flagship or long-horizon-agentic tier for legs that are themselves genuinely ambiguous, hard-to-reverse, or long-running. Model tier is one lever; effort is the other — Anthropic's own effort-level table names "subagents" as the typical use case for `low` effort, so a dispatched worker's default should be `low` regardless of which model tier it runs on, raised only for a leg that turns out to need more depth.
- **You dispatch readily too.** Anthropic's Fable 5 guidance says the model "dispatches parallel subagents more readily than prior models" and recommends explicit guidance on when delegation is appropriate — the decide-first check above applies to you, not just to Opus-class controllers.
- **Keep working while subagents run.** Documented for Fable 5.1: the lead agent often chooses to wait on each subagent even when it could carry on. If your harness returns from a dispatch immediately, do the independent work in the meantime and pick up each result when it arrives.
...
- **The coordinator gets a narrow brief too**, same as any subagent — an open-ended "manage this" mandate drifts into scope creep for a coordinator exactly like it does for a worker. State the coordinator's goal and constraints and let it work out its own sequencing rather than scripting every handoff for it — it's well suited to carrying a multi-step coordination job on its own judgment once the goal is clear.
...
- **Route the coordinator's status traffic to a file/log**, not back into your live context — this matters more than usual here, since a coordinator running quietly by default leaves little else to check in on besides that file.
...
- **Verified, not theoretical:** confirm the mechanism actually works in your environment before relying on it for anything that matters.
- Use this for the reusable-template-on-a-schedule case (a recurring brief that re-fires itself), or for a check-in on long-running work instead of manually deciding when to look again. You're well suited to being on the receiving end of one of these — a fresh unattended turn that has to pick up a long or open-ended task and carry it a long way on its own is exactly the kind of long-horizon work you handle well without a human steering each step.
...
- Give **the task + interfaces it touches + constraints** and let the subagent work out the actual steps. A brief written as a prescriptive, step-by-step script tends to underperform on this model — spelling out "first do A, then B, then C" caps the work at whatever the briefer thought of, when the subagent's own reasoning about how to get there would often find a better path. State the goal and the boundaries; don't hand it a procedure.
- Don't paste a summarized session history — hand over file paths and let the subagent read them itself.
- Pasted text or a returned summary sits in your context from that point on. Passing a file path is always cheaper.
- If the target is vague ("fix this up"), narrow it yourself before delegating — a vague brief comes back as a vague result. Narrowing the *goal* is different from scripting the *steps*: tighten what "done" means, not how to get there.
...
- **If the same kind of task recurs, capture the brief once as a reusable template** instead of re-explaining it from scratch each time — a goal-and-constraints template, not a script. If your environment has a real scheduler, wire the template into one instead of manually re-dispatching it each time it comes up.

## Verify what comes back

- **Don't take the report at face value.** "Tests passed" is a claim, not evidence — run it yourself or read the diff.
...
- **Ask explicitly for the report to include what it tried and rejected, not just what worked.** Left to its own default, a subagent on this model tends to hand back a clean final answer with fewer intermediate notes — if you need to see the path it took to verify the result properly, that has to be part of the brief.
...
## Give long-running work a name and a status line — this one matters especially here
...
- **This is the sharpest gap to plan around on this model.** You write noticeably fewer user-facing updates between tool calls during a long agentic run than other models do by default — which is efficient, but means a long delegated task can look identical to a stuck one from the outside unless something is explicitly writing progress somewhere visible.
- **In many harnesses there's no live channel for this** — a dispatched agent returns one final result, not a stream of intermediate updates you can watch. The only way to get a status line that actually updates mid-run is to brief the agent to write its own progress to a file (e.g. `.claude/status/<name>.md`) and glance at that file yourself — and if you're briefing a worker running on this model, say explicitly that you want that file updated periodically, since it won't volunteer running commentary on its own.
...
- **Mistaking quiet-but-working for stuck**, or the reverse — interpret "no updates in a while" correctly (normal for this model) rather than either panicking or, worse, assuming silence always means fine when a status file would have told you otherwise.
