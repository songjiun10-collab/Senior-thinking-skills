...
- **Opus-class models delegate more eagerly than earlier ones, including for tasks a direct grep or file read would settle faster** — worth knowing if a worker you dispatch is running Opus-class, since its own instinct there will already lean toward delegating further; the "decide whether to delegate first" check applies at every level of the chain, not just the top.
- **If your dispatch tooling responds to trigger phrasing rather than intent alone, be explicit about it.** Whether a given ambiguous request should trigger a subagent dispatch versus a direct action isn't something to leave implicit — say plainly when you mean "dispatch a subagent for this" versus "do this yourself," rather than relying on softer cues that may or may not register as the trigger.
...
- **You track your own remaining context budget as you go** — use that when deciding whether to keep a long-running delegation's status in your live context or push it to a file now, before the budget actually gets tight.
