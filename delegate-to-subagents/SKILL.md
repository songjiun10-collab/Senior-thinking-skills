...
- **If the same kind of task recurs, capture the brief once as a reusable template** instead of re-explaining it from scratch each time — and fold in corrections as they come up, so the template actually improves with use instead of staying a stale first draft. If your environment has a real scheduler (a cron-style trigger that re-fires a saved prompt), wire the template into one instead of manually re-dispatching it each time it comes up — same end state as "demonstrate a workflow once, replay it on a schedule," just built from an explicit saved prompt rather than something the agent is assumed to remember on its own.
