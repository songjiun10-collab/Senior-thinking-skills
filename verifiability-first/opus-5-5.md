...
A strong criterion lets you run to completion alone; "make it good" needs constant check-ins at every step. This is the highest-leverage habit for you specifically: Opus 5 guidance (carried over to 5.5) says the model verifies its own work unprompted once work is underway, so the value isn't in adding more re-checks after the fact — it's in pinning the criterion *before* results exist, so there's nothing to rationalize around later. Thinking is always on and effort defaults to `medium`; when the criterion itself needs real deliberation — a benchmark threshold, a "better" that has to be defined precisely — set effort explicitly higher for that step, since lower effort reduces thinking (and doesn't reliably shrink output — carried over from Opus 5 guidance). Effort is the caller's per-request setting.
