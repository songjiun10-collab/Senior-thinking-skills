...
- When a task is multi-step and expected to outlast one sitting, set up the three files explicitly rather than assuming the instruction to do so is implied.
- This tier is a solid default for running a plan-on-disk workflow inside a larger agentic pipeline — reading, updating checkboxes, appending findings and progress is exactly the repeated, moderate-complexity work it's suited to at its cost point.
- **Reasoning effort doesn't need to sit at the top setting for routine plan updates** — checking a box, appending a finding, logging a progress line are cheap operations; save higher effort for the harder judgment calls the plan surfaces (deciding a phase is genuinely blocked, resolving conflicting findings).
- **Don't let mid-run effort or tool-set changes make you skip re-reading the plan.** Effort and tool-set changes mid-conversation no longer force a fresh start the way they once did, so there's no cost-based reason to defer re-reading `task_plan.md` before a decision just because the run's settings shifted partway through.
- For a plan whose phases border a safety- or access-relevant decision (a phase that touches credentials, production data, or an irreversible action), keep an independent check on that phase's completion rather than trusting a self-reported "done" — this tier's guardrail judgment under autonomous, unsupervised operation is a step behind the top tier's.
