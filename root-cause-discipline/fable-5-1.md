...
The steps below are the shape of a root-cause investigation, not a script to execute mechanically in lockstep. State the investigation's goal — find the actual cause, not the fastest-looking fix — and let the reasoning work through it; this model's own investigative reasoning across a long debugging session tends to outperform a rigidly prescribed sequence, especially on long-horizon bugs that take many steps to pin down.
...
This kind of multi-layer investigation can run long. This model writes fewer intermediate progress updates by default — if visibility into which layer is being checked, and why, matters as the investigation runs, ask for it explicitly.
