...
This tier fits a narrow, well-specified bug — a single failing function, a clear error message, a small reproducible case. A bug spanning multiple services, an ambiguous intermittent failure, or one that's already through two failed fix attempts is judgment-heavy work this tier isn't the right fit for; hand it to a stronger tier rather than pushing through here on higher effort.
...
Just run this check — don't ask first, it's cheap and always worth it.
...
If the bug already looks like it spans multiple layers or services, that's a sign to hand it up rather than instrument all of it here — this tier is best used once the failing layer is already narrowed down.
...
Assumptions about codebase behavior, library behavior, or an error's cause are frequently wrong. If confirming is cheap, confirm it — for a narrow, well-specified bug this is usually a quick, decisive check.
...
- **3 or more** → stop. Don't attempt a fourth fix. On this tier, escalate to a stronger model at the first failed attempt on anything that isn't trivially small — a third failed pass here is a stronger sign to hand off than to keep trying.
