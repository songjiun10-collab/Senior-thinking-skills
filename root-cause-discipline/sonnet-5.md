...
This model follows instructions more literally than the prior generation — if the process below matters end to end (all four numbered phases, not just "find and fix it"), say so explicitly rather than assuming the full discipline will be inferred from a short prompt.
...
Start fixing without knowing where it breaks, and you end up fixing three layers that were never broken. With 1M context, there's room to pull in every layer's logs and recent history for a large multi-service bug in one pass rather than working one file at a time — but name explicitly which layers to instrument, since underspecified scope tends to get read narrowly.
