...
Name explicitly that the full process — all four numbered phases — applies, not just "find and fix it"; a short prompt tends to get done narrowly. Reasoning effort is tunable per call; a routine bug doesn't need `max` to work through the phases correctly, but don't drop effort so low that step 1 (read the full trace) gets skipped — tune by whether the investigation actually lands, not by default.
...
Start fixing without knowing where it breaks, and you end up fixing three layers that were never broken. With 1.05M context, pull in every layer's logs and recent history for a multi-service bug in one pass — name which layers to instrument explicitly, since underspecified scope tends to get read narrowly on this tier too.
...
If this bug is being chased inside an unattended agent pipeline, keep an independent check confirming the claimed fix actually runs clean — don't let "fixed" stand on this tier's say-so alone when nobody reviews the diff before it ships.
