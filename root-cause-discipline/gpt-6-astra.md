...
State the completion bar up front: the investigation is done when you've walked the numbered phases relevant to this bug and can name the cause with evidence, not when you've produced a plausible-looking fix. Don't stop partway to ask "should I keep digging?" — just verify it, all the way to a stated cause, and act.
...
Just check it — this step is cheap enough that it doesn't need approval to run.
...
Start fixing without knowing where it breaks, and you end up fixing three layers that were never broken. With 1.05M context, pull in every layer's logs and recent history for a multi-service bug in one pass rather than working file by file — do this yourself rather than waiting to be told to widen the sweep.
...
You test and confirm your own changes as a matter of course — that part doesn't need spelling out. What's worth stating explicitly:
...
At this point, ask the root question: is this structure even right to begin with? Are you holding onto it out of inertia? **Talk to a person before** attempting the next fix — this is the one point in the process where stopping to ask is the right call, not a sign of under-delegating. Principal angle: if the root cause lives in shared or upstream code, other teams may be hitting the same bug blind — fixing it once at the shared layer is worth more than patching every call site that happened to notice.
