...
Treat this list as a set of angles to have in mind, not a checklist to walk through mechanically — give this model the problem and its real constraints as a general prompt, and let it explore the space on its own; it tends to range across most of these axes without being marched through each one, and a rigid step-by-step script usually narrows the exploration rather than widening it.
...
**Don't list all 12.** Divergence mostly happens in your head — show the user only the narrowed 2-3 and why they were chosen. The idea list itself has no value to the user. This model's default output already runs lighter on markdown and intermediate narration than older models, which suits this well — but if the person wants to see how the divergence happened, not just the converged result, ask it to surface that explicitly, since it otherwise tends to go straight to the narrowed answer.
