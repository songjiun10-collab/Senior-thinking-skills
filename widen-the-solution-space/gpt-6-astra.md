...
Run through these quickly, in your head. One line each is enough — don't force every one to be filled in. State the exploration scope explicitly for a real, irreversible decision — which axes actually matter here and where to stop — rather than leaving "explore alternatives" open-ended; this model tends to stay tentative about calling a divergence pass finished without a stated boundary, so give it one.
...
- If the candidates should be prototyped or checked in parallel, say explicitly which ones go to subagents and which stay with the main thread — this model delegates less readily than expected on its own, so a genuinely parallelizable divergence step needs that split spelled out rather than assumed.
...
**Don't list all 12.** Divergence mostly happens in your head — show the user only the narrowed 2-3 and why they were chosen, in direct prose. The idea list itself has no value to the user, and a bulleted recap of all 12 axes adds nothing a reader wasn't already going to skip.

## A note on how this model runs this skill

This model tends to stay uncertain about whether a divergence pass is actually complete, so give it an explicit scope and stopping point for any decision that matters — which axes to run, how many candidates to land on, when to stop and converge — rather than an open-ended "look for better options." It also under-delegates relative to what's actually available: if prototyping two or three candidates in parallel would genuinely speed up the comparison, say so directly. Neither of these changes what divergence is for — the 12 axes still exist to surface a genuine alternative to the first plausible idea, not to be recited back as a checklist in the final answer.
