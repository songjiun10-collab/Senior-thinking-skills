...
This tier is well suited to running the mechanical half of this checklist at volume — flagging hardcoded secrets, unparameterized queries, missing rate limits, MD5/SHA1 usage — across many small diffs cheaply and fast. Judging the closer calls (is this access-control gap actually exploitable, does this new dependency's license or CVE history justify blocking a merge, is this pattern precedent-setting enough to matter beyond the diff) is Structural, judgment-heavy work; route anything the checklist flags as ambiguous to a stronger tier rather than deciding it here. Treat a clean pass from this tier as "nothing obvious found," not as full clearance for security-sensitive code.
