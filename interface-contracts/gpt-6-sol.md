...
- **Tune reasoning effort to the review, don't default to max.** A one-endpoint contract check doesn't need the same effort as auditing a public API touched by a dozen consumers; more effort doesn't reliably produce a better answer here past a certain point, so raise it for genuinely wide or ambiguous surfaces and leave it lower for a routine, well-scoped one.
