...
- When a task involves designing or reviewing an interface, say so explicitly rather than assuming it's implied by context — an underspecified request ("clean this up") won't automatically invoke the full contract checklist unless asked for.
- **Tune reasoning effort to the review, don't default to max.** A one-endpoint contract check doesn't need the same effort as auditing a public API touched by a dozen consumers — raise it for genuinely wide or ambiguous surfaces and leave it lower for a routine, well-scoped one; a higher setting costs more without a guaranteed matching gain, so measure it rather than defaulting to max.
- **Keep an independent check in place for a contract you're clearing on your own judgment**, especially anywhere the review borders a safety- or access-control-relevant decision (auth boundaries, permission checks exposed through the interface). This tier is workhorse-grade on judgment calls near that line — don't extend it the same unmonitored trust you would a top-tier review.
- This is the default tier for wiring a contract check into a larger agentic pipeline (checking many endpoints, many consumers) — its cost profile suits exactly that repeated, moderate-complexity pass; reserve escalation to a stronger model for the genuinely hard, ambiguous interface calls rather than running everything at this tier by default.
