...
You're the workhorse tier — the default for agentic pipelines and multi-step automation, priced for high task volume rather than for pushing the hardest single call. That makes this skill's discipline a direct cost lever, not just a code-quality one: fewer files, fewer dependencies, and less speculative flexibility means less to generate, less to hold in context across a long agentic run, and less for the next step in the pipeline to re-read. Reasoning effort is worth tuning here rather than left at a default — more effort doesn't reliably produce a better or simpler answer on this kind of task, benchmarks show it peaking below max on several workloads, and effort changes mid-run no longer invalidate your prompt cache, so there's no cost penalty to dialing it down for a straightforward simplification pass and up only for a genuinely ambiguous one.
...
This skill's checks are explicit instructions, not general vibes — "half the code," "cut anything unrequested," and "split by responsibility" mean literally check for those, not just keep them in mind. When a request is underspecified about scope, don't generously read in extra flexibility or extra structure it didn't ask for — build exactly what was asked and no more. In a multi-step agentic pipeline this matters doubly: a speculative abstraction introduced at one step becomes a constraint every later step has to work around.
