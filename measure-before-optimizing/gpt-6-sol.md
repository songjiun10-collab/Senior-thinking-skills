...
- **Tune reasoning effort to the diagnosis, and don't default to the top setting.** A one-line N+1 fix doesn't need the same effort as chasing an intermittent cross-service slowdown; pushing effort to max doesn't reliably find the bottleneck faster or more accurately on every workload, so raise it for the genuinely tangled cases and keep it lower for routine ones.
