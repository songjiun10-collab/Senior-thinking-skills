...
For a single, well-specified lookup (one flag, one function signature, one version check), just verify it directly rather than asking first. This tier is well suited to running that narrow check fast and cheap. Deciding whether a library or version choice is safe to become a team-wide or org-wide default is a bigger judgment call than a single lookup — hand that decision to a stronger tier and use this one for the mechanical verification work underneath it (fetching the doc, checking the lockfile, confirming the installed version).
...
If what you found differs from memory, **record that fact** — the next person would otherwise make the same mistake. Keep each verification narrow and concrete: one claim, one source checked, one answer — that's this tier's strength, and a batch of many such checks run in parallel is a good fit for its cost profile.
