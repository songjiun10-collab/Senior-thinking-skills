...
Name explicitly that "verify this" means walking all four steps above, not glancing at a doc page and moving on — an underspecified verification request tends to get done narrowly rather than expanded to the full check. As one leg of an agentic pipeline, this step is cheap and well-bounded: run it and continue rather than surfacing it as a question, and don't default reasoning effort to `max` for a routine lookup — tune to what the check actually needs.
...
If what you found differs from memory, **record that fact** — the next person would otherwise make the same mistake. With 1.05M context, a whole dependency tree's docs and changelogs can be cross-checked in one pass rather than library by library, useful for a broad audit. If this verification feeds an unattended pipeline decision (picking the library or version other services will inherit), have an independent check confirm the finding before it's treated as settled — don't let a single unreviewed lookup become the org default.
