...
Do steps 2-4 directly rather than reaching for a subagent — a grep of the lockfile or a quick fetch of the release notes is faster inline than a delegation round-trip. Opus 5 guidance (carried over to 5.5) documents delegating more readily than earlier models, including for checks that a direct read settles immediately; reserve dispatch for genuinely large or parallel verification sweeps (e.g. auditing a dozen dependencies at once), not a single flag lookup.
...
Thinking is always on for this model, but effort is the actual depth control, and it defaults to `medium`. At `medium`, what makes a verification pass shallow is skipping the step that confirms the signature actually matches the installed version — so do that step explicitly rather than counting on a higher effort setting (which only the caller can change, per request). Opus 5 guidance (still applicable to 5.5) says the model self-verifies unprompted and that added re-check instructions cause over-verification; don't add a redundant "are you sure the docs say that" re-check on top — that's wasted latency, not a stronger answer.
