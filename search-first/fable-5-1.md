...
A verification sweep across many libraries or a long dependency audit can run for a while with few visible updates in between — this model writes fewer progress notes by default during long agentic passes. Ask explicitly if you want to see which library it's checking as it goes, rather than only the final summary.
