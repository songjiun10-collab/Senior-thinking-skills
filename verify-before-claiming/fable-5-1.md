...
Skip any step and it's not verification — it's a guess. State plainly which command was run and what its output actually showed, not just the conclusion — this model tends to write fewer intermediate updates during a task by default, and the evidence trail is exactly what shouldn't get compressed away into a bare "done."
