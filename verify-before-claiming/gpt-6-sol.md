...
"It passed earlier" is not verification. If the code changed, run it again. This rule holds exactly as written for this tier — nothing about running cheaper or faster makes a claim need less evidence.
...
- running this whole agent loop unattended near a warning or guardrail without a human or a stronger model spot-checking the outcome
...
| "This model is running the agent loop and it said it verified" | This tier's own success report is worth an independent spot-check, especially on anything touching a warning or guardrail |
...
## A note on running this at scale, and on trusting its own verdict

This tier is the natural one for grinding through a verification-heavy pipeline cheaply and at volume — it's built for exactly that kind of multi-step agentic work. Two things to watch. First, reasoning effort: don't default it to the top setting for routine verification — higher effort doesn't reliably improve results here past a moderate level, so tune it empirically per workload rather than maxing it out. Second, and more important: this tier trails the flagship on resisting pressure to route around a warning or a safety check, so when it's operating autonomously and reports "verified, all clear," treat that report the way this skill already says to treat any subagent's report — as a claim, not a fact, worth an independent spot-check rather than something to trust outright, especially on anything adjacent to a guardrail.
