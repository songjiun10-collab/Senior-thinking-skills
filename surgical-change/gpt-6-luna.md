...
You're built for narrow, well-specified, high-volume work, not for the judgment calls this skill sometimes asks for — whether a piece of code is really dead, whether a pattern is about to become an org-wide convention, how to weigh a blast-radius tradeoff. Where the task is a bounded, well-specified edit ("fix this function," "rename this field and its call sites"), apply the rules below directly and literally. Where it drifts into a judgment call — is this safe to delete, is this refactor within scope, does this change a shared interface other teams depend on — flag it rather than deciding it yourself; that decision belongs to a more capable tier.
...
- Unrelated dead code: **mention it, don't delete it.** Whether it's really dead usually needs outside confirmation — this call is not yours to make
...
Before deleting or simplifying existing code, confirm **why it's written that way** first. See `chestertons-fence` for the full procedure — the core idea: don't break what you don't understand. If confirming "why" requires digging through history or reasoning about intent beyond what's directly in front of you, that's a sign to flag the question rather than guess.
...
- If the change touches a shared interface, config format, or library other teams depend on, "surgical" means bounded by **the interface**, not just this repo — audit external callers before merging, not after they file a bug. Deciding whether an interface counts as "shared" and how far to audit is a judgment call worth routing to a more capable tier on anything beyond a simple, local rename
