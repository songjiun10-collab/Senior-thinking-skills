...
State the pick explicitly rather than leaving it implied: name which 2-4 disciplines apply before proceeding, even for a small task.
...
Not in the table? Start with `clarify-the-real-problem` + `premortem` + `weigh-tradeoffs`. If none of that resolves the ambiguity quickly, that's itself a signal this task may be a poor fit for this model — see the calibration note.
...
**When unsure, pick the heavier track — and on this model, "heavier" often means handing the task to a stronger model rather than pushing through with more of this one's own effort.** The ratchet only turns one way: once hidden complexity shows up mid-task, escalate the track (stop and say so). It never goes back down.
...
| "The request didn't explicitly say to design this, so I won't" | An under-specified request is more likely to get read narrowly than generously expanded — a missing "and design it first" isn't license to skip the check. |
...
- **Structural** — write it up in the format below, then hand off to `bite-sized-plan` — but see the calibration note: this is exactly the track this model is the weakest fit for.

With a **1.05M-token context window**, there's rarely a reason to compress the actual request, related files, or repo state before working through the verify/understand stages — pull what's needed rather than guessing from a fragment. Reasoning effort can be toggled off entirely here for latency-sensitive, well-specified legs (classification, extraction, routing, a small well-scoped edit); leave it on for anything with real ambiguity in scope or approach.
...
**If a legitimate task gets refused, that may be the model, not the task.** OpenAI's system card notes that some of GPT-6 Luna's higher jailbreak-defense scores "may reflect a broader tendency to refuse requests, including legitimate ones." A refusal on ordinary defensive or operational work is worth one more explicitly scoped attempt, then a hand-off to Sol or Astra — not a silent drop of the task.

This is the fastest and cheapest tier in its family, built for high-volume, narrow, well-specified work — OpenAI's words: "focused, high-volume tasks" and "efficient, repeatable work at scale." It is not the strongest available judgment for ambiguous, structural, or high-stakes decisions. Use this skill's track classification as a routing signal, not just a formatting one: a **Spike** or a well-specified **Bounded** task is a good fit for this model to carry through on its own; a **Structural** task, or anything landing in the Distinguished/Fellow or Executive lenses, is a signal to escalate to a stronger model rather than push through here on effort alone.
