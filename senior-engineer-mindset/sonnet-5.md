...
This skill is **the router that picks which discipline to pull at each stage.** The actual content lives in each sub-skill. Don't apply all of them — pick the 2-4 that actually bear on this task. State that pick explicitly rather than leaving it implied: this model follows what's actually written more literally than the prior generation, so an unstated "and obviously also check X" doesn't reliably get picked up the way it might have before — if a discipline matters, name it.
...
**This model's tool-use trigger sensitivity differs from the prior generation** — a sub-skill that's meant to fire automatically on a situational cue (e.g. "run `search-first` whenever touching an external API") is more reliable when the trigger wording is explicit and concrete rather than assumed obvious. When wiring these into a workflow or hook, prefer naming the exact situation over a vague cue.
...
| "The request didn't explicitly say to design this, so I won't" | An under-specified request is more likely to get read narrowly on this model than generously expanded — a missing "and design it first" isn't license to skip the check; the trap above still applies regardless of whether the request spelled it out. |
