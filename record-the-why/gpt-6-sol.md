...
Spell out the three required parts (decision, rejected alternatives, reasoning) rather than trusting a short prompt to expand into them on its own. As one step in a larger agentic flow, this is a cheap, well-bounded leg — write the record and move to the next step; it doesn't need high reasoning effort to get right.
...
If this runs unattended as part of a pipeline and the record touches a decision with real downstream weight (an API contract, a security-relevant tradeoff), have a second pass — human or another check — confirm the reasoning actually holds before it's treated as the final word; this tier's judgment near consequential calls is worth double-checking rather than taking on faith.
