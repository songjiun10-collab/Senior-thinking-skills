...
State the completion bar for the record up front: decision, rejected alternatives, and reasoning, written down, not left to be inferred from a short "record the why" prompt. Write the record as part of finishing the task rather than pausing to ask whether one is wanted — if the decision qualifies (expensive to reverse, a reversal, an unnatural-looking workaround), write it and move on.
...
Keep the record itself in plain, direct prose — state the decision and the reasoning as facts, not as a hedge or a comparison framed as "we chose X, not Y." A short paragraph in active voice beats a bulleted list unless the rejected alternatives are genuinely parallel enough that a list actually clarifies the comparison.
